...
- 执行安装
  ...
  - 不升级内核
   ...
   ```
   docker exec -it ansible \
     ansible-playbook -i inventory.ini -e @example/variables.yaml 99-reset-cluster.yml
   ```
   `重新部署前，建议清空etcd数据（/var/lib/etcd/）和k8s配置文件（/etc/kubernetes/）`
